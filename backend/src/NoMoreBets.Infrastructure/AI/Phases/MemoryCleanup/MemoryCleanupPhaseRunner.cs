  ...
  ILogger<MemoryCleanupPhaseRunner> logger)
{
  ...
  {
    var phaseName = MemoryCleanupPhaseDefinition.Phase.ToString();
    logger.LogInformation("Betting agent phase {Phase} starting", phaseName);

    var startedAt = DateTime.UtcNow;
    var sessionId = await unitOfWork.AgentSessions
      .CreateSessionAsync(MemoryCleanupPhaseDefinition.Phase, startedAt, cancellationToken)
