...
public sealed class GetLeagueTableHandler(ILeagueRepository leagues, ILogger<GetLeagueTableHandler>? logger = null) : IRequestHandler<GetLeagueTableQuery, IReadOnlyList<LeagueTableStanding>?>
{
  ...
  {
    var standings = await leagues.GetLeagueTableAsOfAsync(request.LeagueId, request.AsOfDate, cancellationToken).ConfigureAwait(false);
