...
public class UpdateDailySummaryHandlerTests
{
  ...
  public UpdateDailySummaryHandlerTests()
  {
    ...
    _sut = new UpdateDailySummaryHandler(_clubOverviewProvider, _clubs, _unitOfWork, _logger);
  }

  [Fact]
  public async Task Handle_WhenClubNotFound_ReturnsWithoutSaving()
  {
    _clubs.GetByIdAsync(Arg.Any<int>(), Arg.Any<CancellationToken>()).Returns(Task.FromResult<NoMoreBets.Domain.Clubs.Club?>(null));

    var result = await _sut.Handle(new UpdateDailySummaryCommand(42, "Summary text"), CancellationToken.None);

    result.Should().Be(Unit.Value);
    await _clubs.DidNotReceive().AddDailySummaryAsync(Arg.Any<ClubDailySummary>(), Arg.Any<CancellationToken>());
  ...
  public async Task Handle_WhenLatestSummarySameAsRequest_SkipsInsert()
  {
    ...
    _clubs.GetByIdAsync(1, Arg.Any<CancellationToken>()).Returns(Task.FromResult<NoMoreBets.Domain.Clubs.Club?>(club));
    var latestSummary = new ClubDailySummary { Id = 1, ClubId = 1, Summary = "Same summary" };
    _clubs.GetDailySummaryAsync(1, null, Arg.Any<CancellationToken>()).Returns(latestSummary);
  ...
  public async Task Handle_WhenSummaryDifferent_AddsDailySummaryAndSaveChanges()
  {
    var club = new NoMoreBets.Domain.Clubs.Club { Id = 1, Name = "Arsenal", SoccerdataId = 1 };
    _clubs.GetByIdAsync(1, Arg.Any<CancellationToken>()).Returns(Task.FromResult<NoMoreBets.Domain.Clubs.Club?>(club));
    _clubs.GetDailySummaryAsync(1, null, Arg.Any<CancellationToken>()).Returns((ClubDailySummary?)null);

    await _sut.Handle(new UpdateDailySummaryCommand(1, "New summary"), CancellationToken.None);

    await _clubs.Received(1).AddDailySummaryAsync(
