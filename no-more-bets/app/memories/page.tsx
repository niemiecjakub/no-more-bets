...
export default function MemoriesPage() {
    ...
    const sortedMemories = useMemo(() => [...memories].sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()), [memories]);
    ...
    const selectedMemory = selectedId != null ? sortedMemories.find((m) => m.id === selectedId) : undefined;

    return (
        <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
            <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
                <h1 className="mb-6 text-2xl font-semibold tracking-tight text-foreground">Memories</h1>
                {isLoading && memories.length === 0 ? (
                    ...
                    <p className="rounded-lg border border-zinc-200 bg-white px-4 py-6 text-center text-zinc-600 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-400">No memories saved yet.</p>
                ) : (
                                ...
                                {sortedMemories.map((m) => {
                                    ...
                                    return (
                                        <li key={m.id} className="min-w-0">
                                            ...
                                            >
                                                <span className="line-clamp-2 min-w-0 max-w-full wrap-break-word break-all font-medium text-foreground">{m.name}</span>
                                                {m.description ? (
                                                    <span className="mt-1 line-clamp-2 block min-w-0 max-w-full wrap-break-word text-sm text-zinc-600 dark:text-zinc-400">{m.description}</span>
                                                ) : null}
                                                <span className="mt-2 block min-w-0 max-w-full truncate text-xs text-zinc-500 dark:text-zinc-500">Updated {formatDate(m.updatedAt)}</span>
                                            </button>
                                        </li>
                                    );
                                })}
                            ...
                            {selectedMemory ? (
                                <>
                                    ...
                                    <div className="min-h-0 flex-1 overflow-y-auto px-4 py-3">
                                        <pre className="wrap-break-word whitespace-pre-wrap font-mono text-sm text-zinc-800 dark:text-zinc-200">{selectedMemory.content || "—"}</pre>
                                    </div>
                                </>
                            ) : (
                                <div className="flex flex-1 items-center justify-center p-6 text-sm text-zinc-500 dark:text-zinc-400">Select a memory to view its content.</div>
                            )}
                        </div>
                    </div>
                )}
