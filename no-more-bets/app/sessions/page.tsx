...
} from "@/features/bets/services/agent-session-api";
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setLoadError(
            handleServiceError(err, "Failed to load agent sessions."),
          );
        }
      } finally {
        if (!cancelled) {
  ...
  useEffect(() => {
    ...
    void fetchAgentSessionMessages(selectedId)
      ...
      .catch((err) => {
        if (!cancelled) {
          setTranscriptError(
            handleServiceError(err, "Failed to load session transcript."),
          );
        }
      })
      .finally(() => {
        ...
        {isLoadingSessions && sessions.length === 0 ? (
          ...
          <div className="grid min-h-[min(70vh,36rem)] grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,3fr)]">
            <div className="flex min-h-0 min-w-0 flex-col overflow-hidden rounded-lg border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950">
              <div className="shrink-0 border-b border-zinc-100 px-3 py-2 text-xs font-medium uppercase tracking-wide text-zinc-500 dark:border-zinc-800 dark:text-zinc-400">
                Agent sessions
              </div>
